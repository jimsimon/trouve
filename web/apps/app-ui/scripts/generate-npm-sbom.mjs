...
import { isThirdPartyEntry, packageName } from "./lockfile.mjs";
...
const components = Object.entries(lock.packages)
  .filter(([packagePath, metadata]) => isThirdPartyEntry(packagePath, metadata))
...
// The lockfile root is the npm workspace; the SBOM describes this app.
const appManifest = JSON.parse(await readFile(appManifestPath, "utf8"));
const rootName = appManifest.name;
const rootVersion = appManifest.version;
if (typeof rootName !== "string" || typeof rootVersion !== "string") {
  throw new Error("missing app package name or version");
}
