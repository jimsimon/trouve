...
const transcriptSource = readPackageSource("@trouve-ai/transcript/transcript-renderer");
const agentActivitySource = readPackageSource("@trouve-ai/transcript/agent-activity");
...
describe("thread screen asynchronous lifecycle guards", () => {
  ...
  it("delegates elapsed activity updates to an isolated component", () => {
    expect(transcriptSource).toContain('import "./agent-activity.js";');
  ...
  it("keeps timer-only activity text outside polite announcements", () => {
    expect(source).not.toContain("<trouve-agent-activity");
    expect(transcriptSource.match(/<trouve-agent-activity/gu)).toHaveLength(2);
    expect(transcriptSource.match(/\.presentation=\$\{activity\}/gu)).toHaveLength(2);
    expect(transcriptSource.match(/\.input=\$\{activityInput\}/gu)).toHaveLength(2);
