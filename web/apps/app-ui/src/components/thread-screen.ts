...
} from "@trouve-ai/transcript/attachments";
import type {
...
import { readSignal, withSignalTracking } from "@trouve-ai/ui-foundation/reactivity";
import {
...
import type {
  ...
  ThreadChatItem,
} from "@trouve-ai/transcript/thread-view-model";
import {
...
} from "@trouve-ai/transcript/chat-presentation";
...
import {
  ...
  type AgentChatItem,
  type ChatLayout,
} from "@trouve-ai/transcript/chat-layout";
import {
  segmentTurnSpans,
  turnSegmentId,
  turnSegmentUnitId,
  type TurnSegment,
} from "@trouve-ai/transcript/agent-body-plan";
import {
  sameChatCollapsePreferences,
  type ChatPreferences,
} from "@trouve-ai/transcript/chat-preferences";
import {
  runningAgentActivity,
  type AgentActivityPresentation,
  type RunningAgentActivityInput,
} from "@trouve-ai/transcript/agent-activity-model";
import "@trouve-ai/transcript/agent-activity";
import {
...
import {
  ...
  formatSessionUsage,
} from "@trouve-ai/transcript/composer-usage";
import {
  changeModelOption,
  modelOptionControls,
...
import "@trouve-ai/transcript/turn-metadata";
import {
...
} from "@trouve-ai/transcript/question-wizard";
import {
  nextHorizontalTabIndex,
  rovingTabIndex,
} from "@trouve-ai/ui-foundation/tab-navigation";
...
} from "@trouve-ai/transcript/turn-checkpoint-actions";
import {
  agentTurnLabels,
  renderContextUsage,
  ...
  TranscriptRenderer,
  turnPhaseLabel,
  type TranscriptQuestionsItem,
  type TranscriptToolItem,
} from "@trouve-ai/transcript/transcript-renderer";
import { TurnCompletionAnnouncer } from "@trouve-ai/transcript/turn-announcements";
import {
  Virtualizer,
  type VirtualItem,
  type VirtualWindow,
} from "@trouve-ai/ui-foundation/virtualizer";
...
export class TrouveThreadScreen extends withSignalTracking(LitElement) {
  ...
  readonly #turnAnnouncer = new TurnCompletionAnnouncer();
      ...
      <form class="composer" @submit=${this.#sendMessage}>
        ...
        <div class="composer-controls" aria-label="Composer options">
          ...
          ${thread === undefined
            ? nothing
            : html`
                ${renderContextUsage(contextUsage)}
  ...
  #activeComposerCompletion(
    ...
    const emptyMessage = token.kind === "command"
      ? commands.length === 0
        ? "No skills found. Add one under .agents/skills/<name>/SKILL.md."
  ...
  #renderOptimisticPrompt(optimistic: OptimisticPromptSubmission) {
    this.#transcript.ensureMarkdown();
    return html`
      <article
  ...
  #chatDerived(
    ...
    const collapse = this.#transcript.collapsePreferences();
    if (
      cached !== undefined
    ...
    layout.units.forEach((unit, unitIndex) => {
      ...
      const spans = this.#transcript.planTurnBody(unit, presentation, collapse);
  ...
  #renderChat(
    ...
    const virtualItems: VirtualChatItem[] = unitItems.filter((item) =>
      item.kind !== "unit"
      || item.segment.first
      || this.#transcript.isTurnCardOpen(layout.units[item.unitIndex]));
    ...
    const completionAnnouncement = this.#turnAnnouncer.observe(
      this.threadId,
          ...
          ${virtualItems.length === 0
            ...
            : html`<div
              ...
              if (item.kind === "activity") {
                return html`<div data-virtual-id=${item.id} style=${style}>
                  ${this.#transcript.renderActivityRow(item.presentation, liveActivityInput)}
                </div>`;
              }
  ...
  #createTranscriptRenderer(): TranscriptRenderer {
    ...
    return new TranscriptRenderer({
      element: this,
      get threadId() {
        return screen.threadId;
      },
      requestUpdate: () => this.requestUpdate(),
      requestDisclosureUpdate: () => this.#requestDisclosureUpdate(),
      chatPreferences: () => this.#services.value === undefined
        ? undefined
        : readSignal(this.#services.value.chatPreferences),
      availableModels: () => this.#availableModels(),
      interactionGeneration: () => this.#threadInteractionGeneration,
      isCurrentInteraction: (threadId, generation) =>
        this.#isCurrentThreadInteraction(threadId, generation),
      findTool: (callId) => {
        const services = this.#services.value;
        const store = this.#store.value;
        const threadId = this.threadId;
        if (services === undefined || store === undefined || threadId === "") return undefined;
        return store.threadView(threadId).findTool(callId);
      },
      ...
      markdownContextMenu: {
        capture: (event) => this.#captureMarkdownContextMenuSelection(event),
        open: (event, markdown) => this.#openMarkdownContextMenu(event, markdown),
      },
      interactive: {
        renderCheckpointRule: (boundary, turnRunning) =>
          this.#renderCheckpointRule(boundary, turnRunning),
        renderToolApprovalActions: (item) => this.#renderToolApprovalActions(item),
        toolKeydown: (event, callId) => this.#approvalShortcut(event, callId),
        renderPendingQuestions: (item) => this.#renderPendingQuestions(item),
      },
    });
  }

  #renderToolApprovalActions(item: TranscriptToolItem) {
  ...
  #renderPendingQuestions(item: TranscriptQuestionsItem) {
        ...
        ${review
          ? html`
              <div class="question-step question-review question-step-focus" tabindex="-1">
                ${renderQuestionSummary(pendingQuestionSummary(item.questions, state))}
              </div>
            `
          : question === undefined
            ? nothing
