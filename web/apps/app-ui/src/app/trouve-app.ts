...
import { createSignal, readSignal, withSignalTracking } from "@trouve-ai/ui-foundation/reactivity";
...
import {
  ...
  type ChatFileTarget,
} from "@trouve-ai/content-rendering/chat-file-link";
...
export class TrouveApp extends withSignalTracking(LitElement) {
  ...
  readonly #receiveKnownServerEvent = (event: ProtocolEventEnvelope): void => {
    ...
    if (event.type === "server.model_catalog_changed") {
      ...
      // A models.dev download, a background vendor roster rebuild, or the
      // catalog being lost changes every picker at once; invalidate both
      // snapshots on every transition so pickers never offer models that
      // `/v1/models` no longer exposes.
