...
// Shared foundation sources resolve through their package export map so this
// contract keeps following them if the workspace layout changes again.
const readPackageSource = (specifier: string): string =>
  readFileSync(createRequire(import.meta.url).resolve(specifier), "utf8");
...
describe("Trouve visual contract", () => {
  ...
  const tokens = readPackageSource("@trouve-ai/ui-foundation/styles/tokens.css");
  const themes = readPackageSource("@trouve-ai/ui-foundation/styles/themes.css");
  const base = readPackageSource("@trouve-ai/ui-foundation/styles/base.css");
  const chat = readPackageSource("@trouve-ai/transcript/styles/transcript.css");
  const appOnly = read("./app.css");
  // The desktop loads the shared base and transcript stylesheets ahead of its
  // own; geometry contracts below apply to that composed cascade.
  const app = [base, chat, appOnly].join("\n");
  ...
  const icons = readPackageSource("@trouve-ai/ui-foundation/font-awesome-icon");
  const imagePreview = readPackageSource("@trouve-ai/content-rendering/image-preview");
  const attachments = readPackageSource("@trouve-ai/transcript/attachments");
  const codeView = read("../components/code-view.ts");
  const thread = read("../components/thread-screen.ts");
  const transcript = readPackageSource("@trouve-ai/transcript/transcript-renderer");
  const markdown = readPackageSource("@trouve-ai/content-rendering/markdown-view");
  ...
  it("loads WebAwesome's self-hosted base before compact Trouve overrides", () => {
    ...
    for (const entry of [main, gallery]) {
      ...
      expect(entry.indexOf(baseTheme)).toBeLessThan(entry.indexOf("@trouve-ai/ui-foundation/styles/tokens.css"));
    }
  ...
  it("shares element defaults and transcript rendering through the foundation packages", () => {
    for (const entry of [main, gallery]) {
      const order = [
        "@trouve-ai/ui-foundation/styles/tokens.css",
        "@trouve-ai/ui-foundation/styles/base.css",
        "@trouve-ai/transcript/styles/transcript.css",
        "./styles/app.css",
      ].map((specifier) => entry.indexOf(specifier));
      expect(order.every((index) => index >= 0)).toBe(true);
      expect([...order].sort((first, second) => first - second)).toEqual(order);
    }
    expect(base).toContain("* { box-sizing: border-box; }");
    expect(base).toContain("button, textarea, input, select { font: inherit; }");
    expect(base).toMatch(/button:focus-visible[^{]*\{ outline: 2px solid var\(--trouve-accent\)/);
    expect(base).toContain(".trouve-icon-spin");
    expect(base).toContain(".visually-hidden");
    // The chat markup that @trouve-ai/transcript emits is styled once, in its
    // own package, so every shell that mounts it renders it identically.
    for (const selector of [
      ...
      expect(appOnly).not.toContain(selector);
    }
    expect(appOnly).not.toContain("* { box-sizing: border-box; }");
    expect(appOnly).not.toContain(".visually-hidden {");
    // Interactive desktop additions to the same cards stay with the desktop app.
    for (const selector of [
      ...
      expect(appOnly).toContain(selector);
      expect(chat).not.toContain(selector);
    }
  });
  ...
  it("defines every Trouve semantic variable used by shared visual components", () => {
    const visualSources = [
      app,
      codeView,
      readPackageSource("@trouve-ai/content-rendering/markdown-view"),
  ...
  it("keeps the thread, turn-card, and composer geometry", () => {
    ...
    expect(transcript).toContain("message-body turn-body-stream agent-body-stream turn-timeline");
    expect(transcript).not.toContain("turn-activity-footer");
    expect(transcript).toContain('class="turn-rail-node turn-transient-activity"');
    ...
    expect(transcript).toContain('class="thinking-rail-icon"');
    expect(transcript).toContain('fontAwesomeIcon("brain")');
    expect(transcript).toContain('class="thinking-rail-icon progress-rail-icon"');
    expect(transcript).toContain('fontAwesomeIcon("message")');
    expect(transcript).toContain('className: "turn-transient-spinner"');
    expect(transcript).toContain('renderContextUsage(turnContextUsage, "turn-context-usage")');
    expect(transcript).toContain('class="turn-node-body user-body-stream"');
    expect(transcript).not.toContain("Collapse your message");
    expect(transcript).not.toContain("Collapse agent message");
    expect(transcript).toContain("agent-activity-timeline");
    expect(transcript).toContain('activityRows.length === 1 ? "single-activity" : ""');
    expect(transcript).toContain('"ends-with-expanded-tool-group"');
    expect(transcript).not.toContain("activity-group-status");
    expect(transcript).toContain('class="activity-rail-disclosure"');
    expect(transcript).toContain('className: "activity-rail-disclosure-icon"');
    expect(transcript).toContain('class="tool-inline-status ${item.status}"');
    const toolCase = transcript.slice(
      transcript.indexOf('case "tool":'),
      transcript.indexOf('case "questions":'),
    );
    ...
    expect(transcript).toContain('class="agent-copy-action"');
    expect(transcript).toContain('response ? "Copy assistant response" : "Copy assistant progress"');
  ...
  it("uses a local Font Awesome running-tool spinner with product timing", () => {
    ...
    expect(transcript).toContain('running: "spinner"');
    expect(transcript).toContain('spin: item.status === "running"');
    expect(app).toMatch(
  ...
  it("keeps the session-list status precedence and indicators", () => {
    ...
    expect(sessionList).toMatch(
      /class="session-indicator[\s\S]*class="session-copy"[\s\S]*class="session-pr-badge[\s\S]*class="session-age"/,
    );
