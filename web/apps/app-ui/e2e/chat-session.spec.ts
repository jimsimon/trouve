...
import { parseUnifiedDiff } from "@trouve-ai/content-rendering/diff-parser";
import { SUPPORTED_PROTOCOL_VERSION } from "@trouve-ai/protocol/client";
...
test("the chat log is not a live region and completion is announced once", async ({ page }) => {
  ...
  await page.goto("/");
  await replayHistory(page);
  // Streamed Markdown must not be read piecemeal (or all at once when
  // `aria-busy` clears); the activity label and a dedicated status region
  // carry the concise announcements instead.
  ...
  const announcement = page.locator(".transcript-announcement");
  await expect(announcement).toHaveAttribute("role", "status");
  await expect(announcement).toHaveText("");

  await emitBatch(page, [
    threadEvent(16, { type: "turn.started", turn: 8, mode: "code", model: "test/model" }),
    threadEvent(17, { type: "user.message", turn: 8, content: "Stream it", attachments: [] }),
    threadEvent(18, { type: "turn.admitted", turn: 8, provider_wait_ms: 0 }),
    threadEvent(19, { type: "assistant.delta", turn: 8, text: "First part" }),
    threadEvent(20, { type: "assistant.delta", turn: 8, text: ", second part" }),
  ]);
  await expect(page.locator(".agent-text-block").last()).toContainText("First part, second part");
  await expect(stream).toHaveAttribute("aria-busy", "true");
  await expect(announcement).toHaveText("");

  await emitBatch(page, [
    threadEvent(21, { type: "assistant.message", turn: 8, content: "First part, second part." }),
    threadEvent(22, {
      type: "turn.completed",
      turn: 8,
      usage: { input_tokens: 2, output_tokens: 1 },
