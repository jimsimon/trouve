...
/**
 * The `<option>` rows of a model picker. A persisted selection that is not a
 * catalog row (a provider pin inside an automatic entry, or an id the current
 * catalog no longer lists) gets a leading row so the select can still show it.
 */
export function modelChoices(
  ...
  return html`${supplemental
      ? html`<option value=${supplemental.value}>
...
/** Loading or failure state of the model catalog above a configuration page. */
export function modelCatalogStatus(
  ...
  return modelCatalogStatus
    ? html`<p class="error-text" role="status">${modelCatalogStatus}</p>`
    : nothing;
}
