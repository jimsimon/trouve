...
import type { RepositoryModelOptionChanges, ReviewApi } from "./api";
...
export class RepositoriesPage extends ReviewElement {
  ...
  protected override render(): TemplateResult {
    ...
    return html`${pageHeader({
      ...
      ${modelCatalogStatus(this.modelsLoaded, this.modelsError)}
      <section class="panel">
        <div class="filters">
          <label class="grow">
            Search
...
export class RepositoryEditor extends ReviewElement {
  static override properties = {
    ...
    draft: { state: true },
    changedModelOptions: { state: true },
    busy: { state: true },
  };
  ...
  /** Per-role option maps edited since the last save; untouched maps are not re-sent. */
  private changedModelOptions: RepositoryModelOptionChanges = {};
  private busy = false;
  ...
  protected override updated(): void {
    this.draftEffect.run([JSON.stringify(this.repository)], () => {
      ...
      this.changedModelOptions = {};
    });
  }

  private markModelOptionsChanged(...roles: (keyof RepositoryModelOptionChanges)[]): void {
    const next = { ...this.changedModelOptions };
    for (const role of roles) next[role] = true;
    this.changedModelOptions = next;
  }

  private async persistRepository(
    next: Repository,
    successMessage = "Saved",
    modelOptionChanges: RepositoryModelOptionChanges = this.changedModelOptions,
  ): Promise<void> {
    this.busy = true;
    try {
      await this.api.saveRepository(next, modelOptionChanges);
      this.changedModelOptions = {};
      ...
      >
        <div class="form-grid">
          ...
          <label>
            ...
            <select
              ...
              @change=${(event: Event) => {
                ...
                this.markModelOptionsChanged("coordinator", "router", "analyst");
          ...
          ${modelOptionsSetting({
            ...
            scope: "Coordinator",
            onChange: (options) => {
              this.markModelOptionsChanged("coordinator");
          ...
          <label class=${semanticRouterConfigEnabled ? nothing : "field-disabled"}>
            ...
            <select
              ...
              @change=${(event: Event) => {
                ...
                this.markModelOptionsChanged("router");
                const selectedRouterModel = modelForSelection(
                  models,
            ...
            >
              ...
              ${modelChoices(models, draft.router_model)}
              ${selectValue(modelSelectionValue(models, draft.router_model))}
            </select>
          ...
          <label>
            ...
            <select
              ...
              @change=${(event: Event) => {
                ...
                this.markModelOptionsChanged("analyst");
                const selectedAnalystModel = modelForSelection(
                  models,
            ...
            >
              <option value="">Inherit coordinator/fallback model</option>
              ${modelChoices(models, draft.analyst_model)}
              ${selectValue(modelSelectionValue(models, draft.analyst_model))}
            </select>
                ...
                return html`<div class="persona-execution">
                  ...
                  <label>
                    ...
                    <select
                      ...
                      @change=${(event: Event) => {
                        ...
                        const selectedModel = modelForSelection(
                          models,
                          model || reviewer.model || this.draft.model,
                        );
                    ...
                    >
                      ...
                      ${modelChoices(models, override?.model)}
                      ${selectValue(modelSelectionValue(models, override?.model))}
                    </select>
        ...
        <div class="action-row">
          ...
          ${repository.mode !== "off"
            ? html`<button
                ...
                ?disabled=${busy}
                @click=${() =>
                  // Disabling is reversible, so dormant model options remain
                  // available on re-enable: no option map travels with it.
