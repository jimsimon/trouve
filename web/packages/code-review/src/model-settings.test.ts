...
describe("model catalog selection", () => {
  it("model catalog status keeps refresh failures visible after initial load", () => {
    expect(modelCatalogStatusMessage(true, "refresh failed")).toBe(
      "Model choices may be stale: refresh failed",
    );
    expect(modelCatalogStatusMessage(true, "")).toBeUndefined();
    expect(modelCatalogStatusMessage(false, "")).toMatch(/still loading/u);
  });

  it("automatic aliases and provider pins resolve to their exact picker rows", () => {
    const route = (provider_id: string) => ({ provider_id, provider_model: "gpt-5.6-sol" });
    const models = [
    ...
    expect(modelForSelection(models, "gpt-5.6-sol")).toBe(models[0]);
    expect(modelForSelection(models, "auto/gpt-5.6-sol")).toBe(models[0]);
    expect(modelForSelection(models, "codex/gpt-5.6-sol")).toBe(models[1]);
    expect(modelForSelection(models, "cursor/gpt-5.6-sol")).toBeUndefined();
    expect(modelSelectionValue(models, "gpt-5.6-sol")).toBe("auto/gpt-5.6-sol");
    expect(modelSelectionValue(models, "openai/gpt-5.6-sol")).toBe("openai/gpt-5.6-sol");
    expect(modelSelectionValue(models, "unknown/model")).toBe("unknown/model");
    expect(supplementalModelSelection(models, "openai/gpt-5.6-sol")).toBeUndefined();
    expect(supplementalModelSelection(models, "unknown/model")).toEqual({
      value: "unknown/model",
      kind: "unavailable",
    });
    expect(supplementalModelSelection(models, "gpt-5.6-sol")).toBeUndefined();
  });

  it("a provider pin remains visible against an older automatic-only catalog", () => {
    const models = [
      {
        id: "auto/gpt-5.6-sol",
    ...
    expect(supplementalModelSelection(models, "codex/gpt-5.6-sol")).toEqual({
