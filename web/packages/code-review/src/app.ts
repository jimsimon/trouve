...
export class CodeReviewApp extends ReviewElement {
  static override properties = {
    ...
    modelCatalog: { state: true },
    staticModelError: { state: true },
  ...
  /**
   * Model choices for every configuration page. The static `/models` result
   * paints first; live provider and vendor-CLI discovery (`/model-routes`)
   * replaces it when it resolves. `loaded` gates the pickers, and `error`
   * keeps a failed live refresh visible without hiding the static list.
   */
  private modelCatalog: ModelCatalog = { models: [], loaded: false, error: "" };
  private staticModelError = "";
  ...
  private staticModelLoad: Promise<void> | null = null;
  private modelRouteLoad: Promise<void> | null = null;
  ...
  private readonly modelRouteRetryEffect = this.effect();
  private readonly staticModelRetryEffect = this.effect();
  ...
  private loadModelRoutes(): Promise<void> {
    if (this.modelRouteLoad) return this.modelRouteLoad;
    const request = this.client
      .getModelRoutes()
      .then((models) => {
        this.modelCatalog = { models, loaded: true, error: "" };
      })
      .catch((cause) => {
        this.modelCatalog = { ...this.modelCatalog, error: errorMessage(cause) };
      });
    this.modelRouteLoad = request;
    void request.finally(() => {
      if (this.modelRouteLoad === request) this.modelRouteLoad = null;
    });
    return request;
  }

  private loadStaticModels(): Promise<void> {
    if (this.staticModelLoad) return this.staticModelLoad;
    const request = this.client
      .getModels()
      .then((models) => {
        this.staticModelError = "";
        const current = this.modelCatalog;
        this.modelCatalog = current.loaded ? current : { ...current, models, loaded: true };
      })
      .catch((cause) => {
        this.staticModelError = errorMessage(cause);
      });
    this.staticModelLoad = request;
    void request.finally(() => {
      if (this.staticModelLoad === request) this.staticModelLoad = null;
    });
    return request;
  }
  ...
  private get isConfigurationRoute(): boolean {
  ...
  protected override updated(): void {
    ...
    const isConfigurationRoute = this.isConfigurationRoute;
    this.configurationEffect.run([isConfigurationRoute], () => {
      if (isConfigurationRoute) void this.loadConfiguration();
    });
    ...
    this.configurationRetryEffect.run([configurationError, isConfigurationRoute], () => {
    ...
    const modelRouteError = this.modelCatalog.error;
    this.modelRouteRetryEffect.run([modelRouteError, isConfigurationRoute], () => {
      if (!isConfigurationRoute || !modelRouteError) return;
      const timer = window.setInterval(() => {
        if (document.visibilityState === "visible") void this.loadModelRoutes();
      }, AUTOMATIC_RETRY_MS);
      return () => window.clearInterval(timer);
    });
    const staticModelError = this.staticModelError;
    this.staticModelRetryEffect.run([staticModelError, isConfigurationRoute], () => {
      if (!isConfigurationRoute || !staticModelError) return;
      const timer = window.setInterval(() => {
        if (document.visibilityState === "visible") void this.loadStaticModels();
      }, AUTOMATIC_RETRY_MS);
      return () => window.clearInterval(timer);
    });
  ...
  private renderPage(dashboard: Dashboard): TemplateResult | typeof nothing {
    ...
    const modelsLoaded = this.modelCatalog.loaded;
    const modelsError = this.modelCatalog.error || this.staticModelError;
  ...
  protected override render(): TemplateResult {
    ...
    const route = this.route;
    const error = this.dashboardError || (this.isConfigurationRoute ? this.configurationError : "");
