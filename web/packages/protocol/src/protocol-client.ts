...
export const SUPPORTED_PROTOCOL_VERSION = "9.10";
...
export class ProtocolClient {
  ...
  async threadToolDetails(
    ...
    let response: Response;
    try {
      response = await this.#fetch(this.#protocolUrl(
  ...
  async serverEvents(options: {
    ...
    return new CursorEventStream({
      path: this.#protocolUrl("/v1/events").href,
      origin: this.#baseUrl,
      after: options.after,
      parse,
