...
/**
 * Run `onChange` (asynchronously, once per batch of writes) whenever the
 * signal's value changes. For imperative hosts outside Lit's render cycle,
 * such as syncing `data-theme` on the document element. Returns a disposer.
 */
export const observeSignal = <T>(
  signal: ReadonlySignal<T>,
  onChange: (value: T) => void,
): (() => void) => {
  const source = new Signal.Computed(() => signal.get());
  let active = true;
  let last = source.get();
