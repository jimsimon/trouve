...
use semble::chunk::{chunk_lines, chunk_source, DESIRED_CHUNK_LENGTH};

fn assert_boundary_invariants(boundaries: &[semble::chunk::ChunkBoundary], text_len: usize) {
    for b in boundaries {
...
proptest! {
    ...
    fn tokenize_roundtrip_is_lowercase(text in "[A-Za-z0-9_ ]{0,200}") {
        for token in semble::tokens::tokenize(&text) {
