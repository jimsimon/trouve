...
const MAX_BRIDGE_TOKEN_BYTES: u64 = 16 * 1024;
...
struct BridgePool {
    ...
    next_thread_gate_sweep: AtomicUsize,
    lifecycle: RwLock<()>,
    closed: AtomicBool,
...
impl Default for BridgePool {
    fn default() -> Self {
        Self {
            ...
            next_thread_gate_sweep: AtomicUsize::new(MIN_THREAD_GATE_SWEEP),
...
impl BridgePool {
    ...
    async fn thread_gate(&self, thread_id: &str) -> Arc<Mutex<()>> {
        ...
        if gates.len() >= self.next_thread_gate_sweep.load(Ordering::Relaxed) {
            gates.retain(|_, gate| gate.strong_count() > 0);
            // Sweep geometrically with the live set. Bursts of distinct
            // queued threads therefore pay linear aggregate cleanup work,
            // while stale weak entries remain bounded by the next threshold.
            self.next_thread_gate_sweep.store(
                gates.len().saturating_mul(2).max(MIN_THREAD_GATE_SWEEP),
                Ordering::Relaxed,
            );
        }
...
async fn run_sdk_turn(
    ...
    *callback.expected_agent_id.write().await = Some(agent_id.clone());
    ...
    if fresh {
        match publish_session_started(&pool.closing, &turn.cancel, events, agent_id.clone()).await {
            Ok(()) => {}
            ...
            Err(SessionPublicationStop::ConsumerClosed) => {
                drop(bridge);
                callback.stop().await;
                pool.terminate_and_remove(&turn.thread_id, &process).await?;
                return Ok(TurnTerminal::ConsumerClosed);
...
fn bridge_token(ready: &ReadyPayload, state_dir: &Path) -> Result<String, BackendError> {
    ...
    let mut file = std::fs::File::open(file).map_err(BackendError::Io)?;
    let metadata = file.metadata().map_err(BackendError::Io)?;
    if !metadata.is_file() {
        return Err(BackendError::Protocol(
            "Cursor SDK Bridge bearer-token path was not a regular file".into(),
        ));
    }
    if metadata.len() > MAX_BRIDGE_TOKEN_BYTES {
        return Err(BackendError::Protocol(format!(
            "Cursor SDK Bridge bearer token exceeded the {MAX_BRIDGE_TOKEN_BYTES}-byte limit"
        )));
    }
    let mut bytes = Vec::with_capacity(metadata.len() as usize);
    file.by_ref()
        .take(MAX_BRIDGE_TOKEN_BYTES + 1)
        .read_to_end(&mut bytes)
        .map_err(BackendError::Io)?;
    if bytes.len() as u64 > MAX_BRIDGE_TOKEN_BYTES {
        return Err(BackendError::Protocol(format!(
            "Cursor SDK Bridge bearer token exceeded the {MAX_BRIDGE_TOKEN_BYTES}-byte limit"
        )));
    }
    let token = String::from_utf8(bytes).map_err(|_| {
        BackendError::Protocol("Cursor SDK Bridge bearer token was not valid UTF-8".into())
    })?;
...
mod tests {
    ...
    fn ready_with_token_file(path: &Path) -> ReadyPayload {
        ReadyPayload {
            schema_version: 1,
            transport: "tcp".into(),
            ...
            auth_token: None,
            auth_token_file: Some(path.to_string_lossy().into_owned()),
        }
    }

    #[test]
    fn bridge_token_file_is_bounded() {
        let runtime = tempfile::tempdir().unwrap();
        let token_file = runtime.path().join("bearer-token");
        std::fs::write(&token_file, " bridge-token\n").unwrap();
        assert_eq!(
            bridge_token(&ready_with_token_file(&token_file), runtime.path()).unwrap(),
            "bridge-token"
        );

        std::fs::write(&token_file, vec![b'x'; MAX_BRIDGE_TOKEN_BYTES as usize + 1]).unwrap();
        assert!(matches!(
            bridge_token(&ready_with_token_file(&token_file), runtime.path()),
            Err(BackendError::Protocol(message)) if message.contains("exceeded")
        ));
    }
    ...
    #[tokio::test]
    async fn thread_gate_cleanup_is_amortized_across_distinct_threads() {
        let pool = BridgePool::default();
        for index in 0..MIN_THREAD_GATE_SWEEP {
            drop(pool.thread_gate(&format!("stale-{index}")).await);
        }
        assert_eq!(pool.thread_gates.lock().await.len(), MIN_THREAD_GATE_SWEEP);

        let live = pool.thread_gate("live").await;
        let gates = pool.thread_gates.lock().await;
        assert_eq!(gates.len(), 1);
        assert!(gates.get("live").and_then(Weak::upgrade).is_some());
        drop(gates);
        drop(live);
        assert_eq!(
            pool.next_thread_gate_sweep.load(Ordering::Relaxed),
            MIN_THREAD_GATE_SWEEP
        );
    }
