...
/// the first envelope makes a long transcript repaint page by page, so its
/// bounded window still appears to stream from the beginning. Treat this as
/// an idle debounce instead: every arriving replay envelope restarts the
...
struct TermState {
    ...
    search_query: String,
    search_match: Option<trouve_slint_terminal::SearchMatch>,
    cursor_shape: i32,
                    ...
                    .follow_thread_events(&thread_id, after, |envelope| {
                        ...
                        if persisted_replay {
                            if replay.lock().unwrap().push(envelope) {
                                ...
                                let id = id.clone();
                                tokio::spawn(async move {
                                    // Do not repaint a long transcript once
                                    // per server replay page. Wait until no
        ...
        ui::set_term(
            ...
            ui::TerminalUiState {
                ...
                mouse_mode: match state.grid.mouse_mode() {
                    trouve_slint_terminal::MouseMode::None => 0,
                    trouve_slint_terminal::MouseMode::Press => 1,
                    trouve_slint_terminal::MouseMode::PressRelease => 2,
                    trouve_slint_terminal::MouseMode::ButtonMotion => 3,
                    trouve_slint_terminal::MouseMode::AnyMotion => 4,
                },
    ...
    async fn handle(&mut self, command: UiCommand) -> Result<()> {
        ...
        match command {
                ...
                if let Some(state) = self.current_term_mut() {
                    if query.is_empty() {
                        ...
                        state.search_match = state.grid.search(
                            &query,
                            if older {
                                trouve_slint_terminal::SearchDirection::Older
                            } else {
                                trouve_slint_terminal::SearchDirection::Newer
                            },
                            skip_current,
                        );
            ...
            UiCommand::TermUnitSelection { row, col, line } => {
                if let Some(range) = self.current_term_mut().and_then(|state| {
                    state.grid.selection_unit(
                        ...
                        col,
                        if line {
                            trouve_slint_terminal::SelectionUnit::Line
                        } else {
                            trouve_slint_terminal::SelectionUnit::Word
                        },
                    )
                }) {
            ...
            UiCommand::TermMouse {
                ...
                let Some((id, bytes)) = self.term_attached().and_then(|(id, state)| {
                    let kind = match kind {
                        0 => trouve_slint_terminal::MouseEventKind::Press,
                        1 => trouve_slint_terminal::MouseEventKind::Release,
                        2 => trouve_slint_terminal::MouseEventKind::Move,
                        3 => trouve_slint_terminal::MouseEventKind::WheelUp,
                        4 => trouve_slint_terminal::MouseEventKind::WheelDown,
                        _ => return None,
                    };
                    let button = match button {
                        1 => trouve_slint_terminal::MouseButton::Left,
                        2 => trouve_slint_terminal::MouseButton::Middle,
                        3 => trouve_slint_terminal::MouseButton::Right,
                        _ => trouve_slint_terminal::MouseButton::None,
                    };
                    let (rows, cols) = state.grid.size();
                    trouve_slint_terminal::encode_mouse(
                        kind,
                        button,
            ...
            UiCommand::TermOutput {
                        ...
                        .find(|(_, state)| state.terminal_id == terminal_id)
                {
                    ...
                    for event in state.grid.drain_events() {
                        match event {
                            trouve_slint_terminal::TerminalEvent::Bell => rang_bell = true,
                            trouve_slint_terminal::TerminalEvent::Title(title) => {
                                if !title.is_empty() {
                                    state.title = title;
                                }
                            }
                            trouve_slint_terminal::TerminalEvent::Resize { rows, cols } => {
                                state.notice =
                                    format!("shell requested {cols}×{rows}; panel size is fixed");
                            }
                            trouve_slint_terminal::TerminalEvent::ClipboardCopy {
                                data, ..
                            } => {
                                use base64::Engine as _;
                            ...
                            trouve_slint_terminal::TerminalEvent::ClipboardPaste { .. } => {
                                state.notice = "blocked terminal clipboard read".into();
                            }
                            trouve_slint_terminal::TerminalEvent::CursorStyle {
                                shape,
                                blinking,
                            } => {
                                state.cursor_shape = match shape {
                                    trouve_slint_terminal::CursorShape::Block => 0,
                                    trouve_slint_terminal::CursorShape::Underline => 1,
                                    trouve_slint_terminal::CursorShape::Bar => 2,
                                };
                                state.cursor_blinking = blinking;
                            }
                            trouve_slint_terminal::TerminalEvent::Hyperlink { .. } => {}
                            trouve_slint_terminal::TerminalEvent::Reply(bytes) => {
                                replies.push(bytes)
                            }
                        }
                    }
    ...
    async fn thread_replay_flush_waits_for_a_complete_quiet_period() {
        ...
        assert!(
            tokio::time::timeout(THREAD_REPLAY_IDLE_FLUSH * 3 / 4, &mut wait)
                .await
                .is_err(),
            "a new replay envelope must restart the idle wait"
        );
        tokio::time::timeout(THREAD_REPLAY_IDLE_FLUSH * 2, wait)
            .await
