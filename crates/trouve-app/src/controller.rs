...
use trouve_slint_media_view::{DecodedMedia, MediaItemData, MediaKind, media_kind};
...
impl Controller {
    ...
    fn render_chat(&mut self, scroll: bool) {
        ...
        let mut rows = rows[range].to_vec();
        ...
        match command {
            ...
            UiCommand::AttachmentFetched { id, kind, result } => {
                ...
                let media = match result {
                    Ok(bytes) => match trouve_slint_media_view::decode(&bytes, kind) {
...
fn make_pending_attachment(
    ...
    let decoded = kind
        .is_viewable()
        .then(|| trouve_slint_media_view::decode(&bytes, kind))
        .transpose();
...
mod tests {
    use super::{
        ChatWindow, QueueEditState, ServerReplayBuffer, SubscriptionRefresh,
        ...
        reorder_id, safe_attachment_name, session_title_fallback, should_open_chat_at_tail,
        thinking_property,
    };
