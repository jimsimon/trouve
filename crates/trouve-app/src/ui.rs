...
use crate::{
    ...
    MediaItem, ModelHealthItem, NavRow, ProviderConfigFieldItem, ProviderItem, QOption, QPair,
    SectionId, TextSegment, ThreadTabItem, TodoUiItem,
};
...
fn to_media_item(item: &trouve_slint_media_view::MediaItemData) -> MediaItem {
...
fn pixels_to_image(pixels: &trouve_slint_media_view::DecodedPixels) -> slint::Image {
...
pub fn set_queue_edit_attachments(ui: &Ui, items: Vec<trouve_slint_media_view::MediaItemData>) {
    let _ = ui.upgrade_in_event_loop(move |ui| {
...
pub fn set_composer_attachments(ui: &Ui, items: Vec<trouve_slint_media_view::MediaItemData>) {
    let _ = ui.upgrade_in_event_loop(move |ui| {
        let rows: Vec<MediaItem> = items.iter().map(to_media_item).collect();
...
pub fn show_gallery(ui: &Ui, items: Vec<trouve_slint_media_view::MediaItemData>, selected: usize) {
    let _ = ui.upgrade_in_event_loop(move |ui| {
        let rows: Vec<MediaItem> = items.iter().map(to_media_item).collect();
...
pub struct TerminalUiState {
    ...
    pub rows: Vec<Vec<trouve_slint_terminal::GridSpan>>,
