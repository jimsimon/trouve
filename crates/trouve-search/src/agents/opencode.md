...
Prefer the trouve tools when they are available — `mcp__trouve-search__search`/`mcp__trouve-search__find_related` (MCP) or `trouve_search`/`trouve_find_related` (native custom tools), depending on which integration is installed. Otherwise use the `trouve-search` CLI via bash.
