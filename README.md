...
the next natural launch. `cargo run -p trouve-app`, other debug builds, and web
preview hosts do not perform self-update checks.

Direct `trouve-server` and `trouve-search` binary installations may replace
