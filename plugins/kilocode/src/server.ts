// Kilo Code plugin exposing trouve code search as native tools.
//
// The plugin keeps a single `trouve` server process alive for the whole
// Kilo session and speaks its newline-delimited JSON-RPC protocol
// directly. That preserves the server's in-process index cache: repeat
// queries — including against remote git URLs — skip index reload
// entirely. No MCP configuration is needed.
//
...
/**
 * Plugin options (set in kilo config as `["kilocode-trouve", {...}]`):
...
export default { id: "kilocode-trouve", server: TrouvePlugin } satisfies PluginModule
