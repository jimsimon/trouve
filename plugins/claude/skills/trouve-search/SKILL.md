---
name: trouve-search
description: Search a codebase by meaning with trouve. Use when looking for where something is implemented, how a feature works, or code related to a known location — instead of Grep or Glob for exploratory or semantic questions.
---
...
Use `search` to find where something is implemented — instead of using Grep
or Glob to discover files. After trouve returns the file and line, navigate
there directly and read that file. Do not grep for the same content again.
...
5. Use Grep only when you need every occurrence of a literal string across
   the whole repo (e.g., all callers of a renamed function).

The index is built on first use and cached; updates are incremental and
shared across branches and worktrees.

## CLI fallback

For sub-agents without MCP access, the `trouve` CLI provides the same
search:
