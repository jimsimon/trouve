...
```bash
trouve search "authentication flow" ./my-project --max-snippet-lines 10  # first 10 lines only, concise
trouve search "save_pretrained" ./my-project                          # full chunk content
trouve search "save model to disk" ./my-project --top-k 10           # more results
```
...
trouve search "authentication" ./my-project --content all
```

Use `trouve find-related` to discover code similar to a known location (pass `file_path` and `line` from a prior search result):
...
If `trouve` is not on `$PATH`, install it with `cargo install trouve` or download a release binary from GitHub.

### Workflow

1. Start with `trouve search` to find relevant chunks. The index is built and cached automatically.
2. Use `--content docs` for documentation, `--content config` for config files, or `--content all` for everything.
3. Navigate directly to the returned file and line. Do not re-search or grep for the same content.
4. Optionally use `trouve find-related` with a promising result's `file_path` and `line` to discover related implementations.
