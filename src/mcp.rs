...
fn call_tool(cache: &mut IndexCache, name: &str, args: &Value) -> String {
    match name {
        "search" => {
            let Some(query) = args.get("query").and_then(|v| v.as_str()) else {
                return "Missing required argument: query".to_string();
            };
            let Some(repo) = args.get("repo").and_then(|v| v.as_str()) else {
                return "Missing required argument: repo".to_string();
            };
        ...
        "find_related" => {
            let Some(file_path) = args.get("file_path").and_then(|v| v.as_str()) else {
                return "Missing required argument: file_path".to_string();
            };
            let Some(line) = args.get("line").and_then(|v| v.as_u64()) else {
                return "Missing required argument: line".to_string();
            };
            let Some(repo) = args.get("repo").and_then(|v| v.as_str()) else {
                return "Missing required argument: repo".to_string();
            };
            ...
            if results.is_empty() {
                json!({"error": format!("No related chunks found for {file_path}:{line}.")})
                    .to_string()
            } else {
                let label = format!("Chunks related to {file_path}:{line}");
                format_results(&label, &results, max_snippet_lines).to_string()
            }
        }
...
fn handle_request(cache: &mut IndexCache, request: &Value) -> Option<Value> {
    ...
    let result = match method {
        ...
        "tools/call" => {
            ...
            let text = call_tool(cache, name, args);
