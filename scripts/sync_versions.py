...
def _sync_nested_manifest(
    root: Path, manifest_path: Path, expected: str, *, check: bool
) -> tuple[set[str], list[str]]:
    """Synchronize one nested manifest and return its local package names."""
    ...
    return internal_names | {package_name}, changes


def _generate_nested_lockfile(nested_root: Path, failure_context: str) -> None:
    result = subprocess.run(
        ["cargo", "generate-lockfile", "--offline"],
        cwd=nested_root,
        capture_output=True,
    ...
    if result.returncode:
        detail = result.stderr.strip() or result.stdout.strip()
        raise VersionSyncError(f"{failure_context}: {detail}")


def _ensure_nested_lockfile(
    root: Path, nested_root: Path, lock_path: Path, *, check: bool
) -> list[str]:
    """Create a missing nested lockfile in write mode and report the drift."""

    if lock_path.exists():
        return []
    changes = [f"{lock_path.relative_to(root)}: missing"]
    if not check:
        _generate_nested_lockfile(
            nested_root,
...
def _nested_local_lock_records(
    lock_path: Path, local_names: set[str]
) -> dict[str, dict]:
...
def _sync_nested_lock_records(
    ...
    *,
    ...
    """Synchronize first-party package versions inside one nested lockfile."""

    local_records = _nested_local_lock_records(lock_path, local_names)
    changes: list[str] = []
    ...
    if missing:
        ...
        if not check:
            _generate_nested_lockfile(
                nested_root,
                "nested cargo generate-lockfile failed while refreshing",
            )
            local_records = _nested_local_lock_records(lock_path, local_names)
...
def _validate_nested_metadata(
    root: Path, manifest_path: Path, expected: str
) -> None:
    """Require Cargo to resolve only the synchronized first-party version."""
...
def sync_nested_cargo_workspaces(
    ...
    for relative in NESTED_CARGO_WORKSPACES:
        ...
        local_names, manifest_changes = _sync_nested_manifest(
            root, manifest_path, expected, check=check
        )
        changes.extend(manifest_changes)
        lock_path = nested_root / "Cargo.lock"
        changes.extend(
            _ensure_nested_lockfile(root, nested_root, lock_path, check=check)
        )
        if not lock_path.exists():
            continue

        changes.extend(
            _sync_nested_lock_records(
                root,
                nested_root,
                lock_path,
                local_names,
        ...
        if check and len(changes) > workspace_change_index:
            continue
        _validate_nested_metadata(root, manifest_path, expected)
