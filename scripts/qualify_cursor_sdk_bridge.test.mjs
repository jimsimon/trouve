...
test("qualification timeouts abort and settle the losing operation", async () => {
  ...
  await assert.rejects(
    withTimeout(operation, 5, "fixture operation", async (timeoutError) => {
      controller.abort(timeoutError);
      await operation.catch(() => {});
    }),
    /fixture operation timed out/u,
  );
  assert.equal(controller.signal.aborted, true);
  assert.equal(settled, true);
});

test("qualification timeout cleanup has an independent deadline", async () => {
  const never = new Promise(() => {});
  await assert.rejects(
    withTimeout(never, 5, "fixture operation", () =>
      withTimeout(never, 5, "fixture cleanup")),
    /qualification failed: fixture operation timed out; cleanup also failed: fixture cleanup timed out/u,
  );
});
...
test("a blocked full qualification returns a failing process status", () => {
  ...
  assert.equal(qualificationExitCode({ decision: "unknown" }), 1);
  assert.equal(qualificationExitCode({ result: "unknown" }), 1);
  assert.equal(qualificationExitCode({}), 1);
  assert.equal(qualificationExitCode(undefined), 1);
});
...
test("an absent native tool event is never reported as an explicit denial", () => {
  assert.deepEqual(nativeToolPromptObservation(false), {
    prompt_requested: true,
    model_attempt_attested: false,
    qualification_status: "corroborating-only",
    explicit_denial_observed: false,
    native_tool_event_observed: false,
  });
});
...
test("full qualification fails closed on an observed Cursor-native tool event", () => {
  assert.throws(
    () => inspectToolCalls(
      [
        {
    ...
    /Cursor-native stream tools appeared: shell/u,
  );
});
