...
export async function withTimeout(promise, milliseconds, label, onTimeout) {
  let timer;
  let timeoutError;
  ...
  try {
    return await Promise.race([
      ...
      new Promise((_, reject) => {
        timer = setTimeout(() => {
          didTimeout = true;
          timeoutError = new QualificationError(`${label} timed out`);
          reject(timeoutError);
        }, milliseconds);
      }),
    ]);
  } catch (error) {
    if (!didTimeout || error !== timeoutError || onTimeout === undefined) throw error;
    try {
      await onTimeout(timeoutError);
    } catch (cleanupError) {
      throw combineQualificationAndCleanupErrors(error, cleanupError);
    }
    throw error;
  } finally {
    clearTimeout(timer);
...
export function inspectToolCalls(frames, callbacks, expectedTools, expectToolError, label) {
  ...
  if (nativeToolEventObserved) {
    throw new QualificationError(
      `Cursor-native stream tools appeared: ${nativeToolNames.join(", ")}`,
    );
  }
  ...
  // assertUniqueToolLifecycle has already rejected every missing or empty id,
  // so retain the complete event set here instead of silently filtering it.
  const streamIds = new Set(toolMessages.map((message) => message.call_id));
...
async function qualifyCancellation({
  ...
  const callbackStarted = blockControl.started.promise.then((record) => {
    callbackRecord = record;
    return record;
  });
  const abortSendAndSettle = async (timeoutError) => {
    sendAbort.abort(timeoutError);
    // The timeout path must be bounded independently of the operation it is
    // cleaning up. Release our handler even if the Bridge left its callback
    // request open, then give both sides a short window to acknowledge before
    // surfacing the original qualification timeout plus any cleanup timeout.
    blockControl.release.resolve({ value: RESULTS.blockReleased });
    await withTimeout(
      Promise.all([
        observedSend,
        callbackRecord?.settled.promise ?? Promise.resolve(),
      ]),
      Math.min(timeoutMilliseconds, 5_000),
      "cancellation probe timeout cleanup",
    );
  };

  const [runId, startedCallback] = await withTimeout(
    Promise.all([runIdReady.promise, callbackStarted]).then(
  ...
  callbackRecord = startedCallback;
  await unary(
    bridge,
    "SdkAgentService",
    "CancelRun",
  ...
  try {
    ...
    qualificationResult = {
      ...
      non_gating_observations: {
        native_shell_prompt: nativeToolPromptObservation(
          readTurn.summary.native_tool_event_observed,
        ),
        plan_mode_request: {
...
export function qualificationExitCode(result) {
  if (result?.decision !== undefined) {
    return result.decision === "proceed-with-sdk-bridge-adapter" ? 0 : 1;
  }
