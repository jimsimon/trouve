...
function requestTimeout(timeoutMilliseconds, parentSignal) {
...
async function serverStream(
  ...
  const { signal, timer } = requestTimeout(timeoutMilliseconds, parentSignal);
  try {
    const response = await fetch(`${client.url}/sdk.v1.${service}/${method}`, {
      method: "POST",
      headers: {
...
async function send(client, request, timeoutMilliseconds, onMessage, parentSignal) {
  return serverStream(
    client,
    "SdkAgentService",
    "Send",
...
// sdk.v1 exposes neither a direct native-tool invocation RPC nor an effective
// tool-inventory attestation. A prompted shell request can falsify confinement
// when a native event appears, but silence cannot prove that the Bridge denied
// the call or that the model attempted it.
function nativeToolPromptObservation(nativeToolEventObserved) {
  return {
    prompt_requested: true,
    model_attempt_attested: false,
    qualification_status: "corroborating-only",
...
async function qualifyTurn(client, callback, agentId, ordinal, timeoutMilliseconds) {
  ...
  // omit it. This is corroborating telemetry only; the contract-shape check
  // rejects an unknown built-in through CreateAgent before paid turns.
  ...
  const nativeToolNames = toolNames.filter(toolIsForbidden);
  const nativeToolEventObserved = nativeToolNames.length > 0;
  if (nativeToolEventObserved) {
    throw new QualificationError(
      `turn ${ordinal}: Cursor-native tools escaped confinement: ${nativeToolNames.join(", ")}`,
    );
  }
...
async function verifyToolAllowlist(client, options, timeoutMilliseconds) {
  ...
  try {
    ...
    return {
      ...
      runtime_enforcement_attestation: "unavailable-from-sdk.v1",
...
async function main() {
  ...
  try {
    ...
    const nativeShellPromptObservation = nativeToolPromptObservation(
      turns.some((turn) => turn.native_tool_event_observed),
    );
    const builtInsConfinedByContract =
      toolPolicy.contract === "@cursor/sdk 1.0.28 AgentOptions.tools allowlist" &&
      toolPolicy.known_native_tool_recognized === KNOWN_NATIVE_TOOL_PROBE &&
      toolPolicy.known_native_probe_agent_run === false &&
      JSON.stringify(toolPolicy.explicit_native_denylist) ===
        JSON.stringify(CURSOR_NATIVE_TOOL_DENYLIST) &&
      toolPolicy.unknown_tool_rejected_with_invalid_argument === true &&
      toolPolicy.model_behavior_used_as_evidence === false &&
      nativeShellPromptObservation.native_tool_event_observed === false;
    if (!builtInsConfinedByContract) {
      throw new QualificationError("Cursor tool-policy contract validation was incomplete");
    }
      ...
      `${JSON.stringify(
        {
          ...
          model,
          built_ins_confined_by_contract: builtInsConfinedByContract,
          ...
          non_gating_observations: {
            native_shell_prompt: nativeShellPromptObservation,
          },
          exactly_once: true,
          resume: true,
