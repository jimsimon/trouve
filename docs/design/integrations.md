...
approval for the first use of each server per session in ask and allow-list
modes (`allow_key` = `mcp:<server>`; a plain Approve unlocks the server for the
rest of the session). Yolo skips all approval prompts, including MCP. Rationale
for the ask/allow-list gate: MCP servers are external code and a
