...
| `crates/trouve-app/ui/settings-window.slint` | `src/components/settings-screen.ts`, provider/mode/local/CLI/workspace/management/code-review settings components | General, Sessions & Chat (including session naming), Providers, Modes & Models, MCP, Integrations, Appearance, Notifications, About, validation, login/device flow, installed fonts, install/download progress, and centered desktop layout are implemented. |
