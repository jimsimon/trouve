...
| `crates/trouve-client-core/src/viewmodel.rs` | `src/state/thread-view-model.ts`, `tool-output.ts`, `src/services/thread-ingress.ts` | All 21 folded event variants and every protocol `ThreadViewItem` snapshot variant map explicitly. Tool output head/tail truncation and UTF-8 safety, approvals/questions, commands, queue, todos, compaction, usage, turn state, idempotency, cursor ordering, folded-page offsets, and bounded buffers are preserved. |
