...
| `CHAT_WINDOW_ROWS`, history refill, stable chat identity, and replay batching in `controller.rs`/`ui.rs`. | Persisted old history could arrive in many small renders and large timelines needed stronger identity/anchor behavior. | The owned virtualizer uses keyed rows, measured anchoring, bounded default DOM, explicit full-history accessibility mode, and a 25 ms replay batch for events older than two seconds. |
...
| Retained source | Corresponding TS/JS source(s) | Disposition |
| --- | --- | --- |
| `crates/trouve-client-core/src/client.rs` | `src/services/protocol-client.ts`, `cursor-event-stream.ts`, `protocol-ingress.ts`, `thread-ingress.ts`; `web/review-ui/src/api.ts` for detailed review jobs | Generated-path HTTP mutations/queries, response validation, safe errors, URL encoding, cursor-bearing snapshots, SSE replay/resume/reconnect, and empty responses are present. App-only review methods are consolidated behind the current review APIs; detailed job/task/stat/event operations remain in the existing review web app. |
...
| `crates/trouve-client-core/src/viewmodel.rs` | `src/state/thread-view-model.ts`, `tool-output.ts`, `src/services/thread-ingress.ts` | All 21 folded event variants match exactly. Tool output head/tail truncation and UTF-8 safety, approvals/questions, commands, queue, todos, compaction, usage, turn state, idempotency, cursor ordering, and bounded buffers are preserved. |
...
| Chat rendering | Keyed virtualization, replay batching, tail anchoring, bookmarks, accessible full history, collapsed-body unmounting, Markdown streaming, raw/formatted output, tool metadata/diffs, processing/activity/status rows, attachments, links, copy. |
