...
| 3. Target architecture | `web/apps/app-ui`, `trouve-desktop-host`, default Wry host, isolated Servo preview, HTTP/SSE state path, typed native boundary, separate desktop/PWA builds. | Cargo boundary tests, protocol/host schema tests, build-mode verifier. | Implemented. |
