...
| 8. Protocol/projections | Generated OpenAPI clients, precompiled CSP-safe runtime validators, atomic session summaries/cursor resume, server event replay, normalized PR/session/thread/todo projections, ephemeral PTY stream. | Schema drift, validator sync/CSP tests, Rust/TS shared projection fixture, ingress race/recovery tests, browser PR projection test. | Implemented for current protocol; live remote-host/security qualification remains. |
