# ADR 0050: One shared Cursor SDK Bridge per backend

Status: Accepted (2026-08)

Supersedes [ADR 0047](0047-cursor-sdk-bridge-transport.md)'s per-thread
...
ADR 0047 isolated one warm Cursor SDK Bridge per Trouve thread because the
...
- Turn cancellation sends `CancelRun` when the run id is available, removes
  ...
  disabled as decided in ADR 0047.
...
- Cursor receives only the `mcp` capability and the complete native-tool
  denylist from ADR 0047. Every callback still enters the thread's internal MCP
...
- **Keep ADR 0047's per-thread process pool.** It preserves physical isolation
