# ADR 0018: Bounded, coalesced event ingestion
...
- Delta chunk boundaries are not semantic. Adjacent text, thinking, and
  ...
  never coalesced.
- Per-turn coalescing buffers remain bounded by message count and approximate
  bytes. Saturation applies ordered backpressure within that turn; an
  indivisible message larger than the byte budget is admitted only to an
  otherwise empty buffer.
- Readers shared by multiple vendor turns remain nonblocking. Their per-turn
  routes have a bounded event budget and report overload explicitly to the
  affected turn instead of silently deleting its route or stalling unrelated
...
- Truly stalled single-turn producers are backpressured within a bounded
  memory budget. Multiplexed transports instead fail only the route that
  exhausts its budget, so unrelated turns continue making progress.
