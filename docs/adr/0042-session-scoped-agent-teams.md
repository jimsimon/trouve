# 0042: Session-scoped role-based agent teams
...
The shared Lit client presents teams as a distinct session kind with a durable
timeline, role and usage roster, mention-aware composer, and lifecycle controls.
Team delivery queues are server-owned rather than directly editable through the
ordinary thread controls. The client observes events and refreshes the
authoritative snapshot; it never coordinates agents locally.
