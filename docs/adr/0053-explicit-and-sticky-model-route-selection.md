# 0053 — Explicit and sticky model route selection

Status: Accepted (2026-08).

## Context

ADR 0052 introduced provider-neutral model selection and bounded failover, but
...
This decision partially supersedes ADR 0052's picker-identifier and
...
- `/v1/model-routes` exposes both `auto/<model>` entries and concrete
  ...
  longer emitted by the picker catalog. `auto` is reserved as a selector
  namespace and cannot be used as a provider id.
