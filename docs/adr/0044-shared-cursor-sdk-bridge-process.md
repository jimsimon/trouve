# ADR 0044: One shared Cursor SDK Bridge per backend

Status: Accepted (2026-08)

Supersedes [ADR 0043](0043-cursor-sdk-bridge-transport.md)'s per-thread
...
ADR 0043 isolated one warm Cursor SDK Bridge per Trouve thread because the
...
- Cursor receives only the `mcp` capability and the complete native-tool
  denylist from ADR 0043. Every callback still enters the thread's internal MCP
...
- **Keep ADR 0043's per-thread process pool.** It preserves physical isolation
